        ...
        <div className="w-full h-screen mt-24 xl:mt-44 flex flex-col items-center">
            ...
            <div className="flex flex-wrap justify-center my-auto">
                {profiles.map((profile, index) => (
                    <div key={index} className="m-4 flex flex-col border border-green-900 rounded-lg relative w-[418px] h-[255px]">
                        {/* BorderBeam for shining border effect */}
                        
                            <BorderBeam />
                        
                        <div className="relative z-10 shadow-md rounded-3xl overflow-hidden flex flex-col h-full p-4 bg-black"> {/* Changed background to black */}
                            {/* Inner content */}
                            <div className="text-center text-2xl font-bold capitalize flex-grow">
                                <div className="flex mb-4">
                                    ...
                                    <div className="flex-grow flex items-center justify-between pl-4">
                                        <div>
                                            <h2 className="text-xl font-bold text-white">{profile.name}</h2> {/* Text color changed to white */}
                                            <p className="text-green-600">{profile.location}</p>
                                        </div>
                                        <div className="flex-shrink-0">
                                            <p className="text-lg font-semibold text-white">Rating: {profile.rating}</p> {/* Text color changed to white */}
                                        </div>
                                    </div>
                                </div>
                                {/* Bottom div with description */}
                                <div className="p-2">
                                    <p className="text-gray-300 text-sm">{profile.description}</p> {/* Text color changed for better contrast */}
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div className="flex justify-between px-44 items-center w-full">
                ...
                {/* RIGHT */}
                <div><span>arrow</span> <span>arrow</span></div>
            </div>
        </div>
    );
};
